...
export default function DeliverableModal({ taskId, taskTitle }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res?.modifiedCount > 0) {
        ...
        toast.error("Failed to submit deliverable: " + JSON.stringify(res));
      }
    } catch (err) {
  ...
  const inputClass =
    "w-full text-white bg-[#141416]/90 border border-zinc-800/80 hover:bg-[#1c1c1f] focus:border-emerald-500/50 rounded-xl h-12 px-4 text-sm placeholder:text-zinc-600 outline-none transition-all duration-200 shadow-inner";

  return (
    <Modal>
      <Button className="w-full bg-gradient-to-r from-emerald-400 via-teal-400 to-emerald-500 text-black font-black text-xs uppercase tracking-wider rounded-xl py-3 hover:brightness-110 transition-all duration-200 shadow-[0_4px_20px_rgba(52,211,153,0.1)] cursor-pointer h-11">
        Submit Deliverable
      </Button>
      ...
      <Modal.Backdrop>
        <Modal.Container placement="auto">
          <Modal.Dialog className="sm:max-w-md bg-[#0E0E12] border border-zinc-800/80">
            ...
            <Modal.Header>
              <Modal.Heading className="text-white font-black">
                Submit Deliverable
              </Modal.Heading>
              <p className="mt-1.5 text-sm leading-5 text-zinc-400">
                Paste your deliverable link below. This will mark{" "}
                <strong className="text-white">{taskTitle}</strong> as
                completed.
              </p>
            </Modal.Header>
            ...
            <Modal.Body className="p-6">
              <div className="flex flex-col gap-2">
                ...
                <p className="text-[10px] text-zinc-600 mt-1">
                  Accepted: GitHub, Google Docs, Figma, Notion, or any public
                  URL
                </p>
              </div>
            </Modal.Body>
